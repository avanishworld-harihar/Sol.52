...
export function DgHybridDiagram({ systemKw, capacityKva, presentation = false, className }: Props) {
  const topNodes = [
    {
      label: "GRID",
      sub: "DISCOM supply",
      cls: "commercial-dg-node commercial-dg-node--grid border-slate-400/50 bg-slate-500/15 text-slate-200",
    },
    {
      label: "LT PANEL",
      sub: "Main distribution",
      cls: "commercial-dg-node commercial-dg-node--lt border-sky-400/50 bg-sky-500/15 text-sky-100",
    },
    {
      label: "HYBRID CONTROLLER",
      sub: "Priority · ATS",
      cls: "commercial-dg-node commercial-dg-node--ctrl border-indigo-400/60 bg-indigo-500/20 text-indigo-100",
    },
  ] as const;
  ...
  return (
    <motion.div
      className={cn(
        "commercial-dg-diagram relative overflow-visible rounded-2xl border border-white/10 bg-gradient-to-b from-slate-900 via-[#0c1220] to-slate-950 p-4 sm:p-6 print:overflow-visible",
    ...
    >
      ...
      <p className="relative mb-4 text-center text-[10px] font-bold uppercase tracking-[0.2em] text-sky-300">
      ...
      <div className="relative mx-auto flex max-w-md flex-col items-center gap-1">
        {topNodes.map((node, i) => (
          <div key={node.label} className="flex flex-col items-center">
            ...
            >
              ...
              <p className="text-[10px] text-white/90">{node.sub}</p>
            </motion.div>
        ...
        <div className="relative flex w-full max-w-sm items-start justify-between gap-2 pt-2">
          <div className="flex flex-col items-center">
            <DiagonalArrow />
            <div className="commercial-dg-node commercial-dg-node--solar mt-1 rounded-xl border border-amber-400/60 bg-amber-500/20 px-3 py-2 text-center text-amber-100">
              <p className="text-[11px] font-bold">{systemKw} kW SOLAR</p>
              <p className="text-[9px] text-white/90">On-site PV</p>
            </div>
          </div>
          <div className="flex flex-col items-center">
            <DiagonalArrow flip />
            <div className="commercial-dg-node commercial-dg-node--genset mt-1 rounded-xl border border-rose-400/60 bg-rose-500/20 px-3 py-2 text-center text-rose-100">
              <p className="text-[11px] font-bold">{capacityKva} kVA DG</p>
              <p className="text-[9px] text-white/90">Critical backup</p>
            </div>
          </div>
        </div>

        <Arrow className="mt-2" />
        <div className="commercial-dg-node commercial-dg-node--loads w-full min-w-[200px] rounded-xl border border-emerald-400/60 bg-emerald-500/20 px-4 py-2.5 text-center text-emerald-100">
          <p className="text-xs font-bold tracking-wide">CRITICAL LOADS</p>
          <p className="text-[10px] text-white/90">Operations · Life safety · IT</p>
        </div>
      </div>
    </motion.div>
  );
