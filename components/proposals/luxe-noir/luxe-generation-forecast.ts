/**
 * Premium Luxe — monthly generation forecast (Canvas seasonal profile)
...
function monthIndexFromLabel(label: string): number | null {
  ...
  // Full English month fallback
  const full = raw.toLowerCase();
  const fullIdx = LUXE_MONTH_SHORT.findIndex((m) =>
    full.startsWith(m.toLowerCase())
  );
  return fullIdx >= 0 ? fullIdx : null;
}

/** Map bill audit months onto Jan–Dec (null = no bill for that month). */
export function mapBillUnitsByCalendarMonth(
  ...
  for (const row of billMonths) {
    ...
    // Prefer the latest / last-seen value if duplicates
    out[idx] = units;
  }
  return out;
}

export function buildLuxeForecastMonths(opts: {
  annualGenUnits: number;
  annualSavingsInr: number;
  billMonths?: ProposalBillMonth[] | null;
  /** When true, include bill bars even if some months are empty. */
  ...
  const genUnitsList = LUXE_MONTH_SHORT.map((_, i) => {
    const share = MP_GEN_SHARE[i]! / shareSum;
    return opts.annualGenUnits > 0
      ? Math.round(opts.annualGenUnits * share)
      : 0;
  });
  ...
  return LUXE_MONTH_SHORT.map((label, i) => {
