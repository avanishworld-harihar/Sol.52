/**
 * Khadi engineering arena — elevated GI MMS table layout.
 *
...
export type KhadiArenaKind = "1P" | "2P" | "3P";

export type KhadiArenaLayout = {
  rows: number;
  cols: number;
  /** Row-major, north (top) → south (bottom). `false` is a reserved gap. */
  cells: boolean[];
  tableKind: KhadiArenaKind;
  heights: number[];
};
...
function pickKind(count: number): KhadiArenaKind {
...
export function khadiArenaLayout(count: number): KhadiArenaLayout {
  const n = Math.max(0, Math.floor(count));
  if (n <= 0) {
    return { rows: 0, cols: 0, cells: [], tableKind: "1P", heights: [] };
  }
  if (n <= 3) {
    const kind: KhadiArenaKind = n === 3 ? "3P" : n === 2 ? "2P" : "1P";
    return {
      rows: n,
      cols: 1,
