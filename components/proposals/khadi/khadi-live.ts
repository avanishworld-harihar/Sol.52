/**
 * Khadi — live ProposalData only. No Harihar / Satna / 5 kW / ₹2.2L / Waaree fallbacks.
 */
...
export const KHADI_HERO_ALT =
  "Oblique morning terrace: Indian family having tea beside a receding elevated GI solar array";
...
export function khadiBrandParts(data: ProposalData): { head: string; tail: string } {
  const brand = khadiBrand(data);
...
export function khadiNetInvestment(data: ProposalData): number {
...
export function khadiAnnualUnits(data: ProposalData): number {
  return data.closing.annualUnits > 0 ? Math.round(data.closing.annualUnits) : 0;
}

export function khadiLocation(data: ProposalData): string {
  return data.meta.locationLine?.trim() || data.engineering.cityLabel?.trim() || "";
}

export function khadiIssueDate(generatedAt?: string): string {
...
export function formatKhadiKw(kw: number): string {
  if (!(kw > 0)) return "—";
  return kw % 1 === 0 ? String(kw) : kw.toFixed(1);
}

export function khadiMonthlySavings(data: ProposalData): number {
...
export function khadiAnnualSavings(data: ProposalData): number {
...
export function khadiYearlyBill(data: ProposalData): number {
...
export function khadiMonthlyBill(data: ProposalData): number {
  ...
  const yearly = khadiYearlyBill(data);
  return yearly > 0 ? Math.round(yearly / 12) : 0;
}

export function khadiBillMonths(data: ProposalData): ProposalData["bill"]["months"] {
  return (data.bill.months ?? []).filter((m) => m.units > 0 || m.netInr > 0);
}

export function khadiLifetime(data: ProposalData): number {
...
export type KhadiHwKind = "panel" | "inverter" | "structure" | "dcdb" | "acdb" | "la" | "earth";

export type KhadiHardwareRow = {
  kind: KhadiHwKind;
  role: string;
  title: string;
  detail: string;
...
const KHADI_HW_CATALOG: Record<KhadiHwKind, KhadiHwCatalog> = {
  panel: {
    kind: "panel",
...
const KHADI_HW_IMAGES: Array<{ match: RegExp; src: string }> = [
...
function khadiHardwareImage(hay: string): string {
  for (const row of KHADI_HW_IMAGES) {
...
function filterPointsForKind(kind: KhadiHwKind, points: string[]): string[] {
...
function rowFromCatalog(
  ...
  const catalog = KHADI_HW_CATALOG[kind];
  ...
  return {
    ...
    image: khadiHardwareImage(hay),
    accent: catalog.accent,
  };
}

/** Typical central-India rooftop share of year-1 yield. Applied only to live annual units. */
const KHADI_MONTH_SHARE: ReadonlyArray<{ m: string; share: number; peak: boolean }> = [
...
function monthIndexFromLabel(label: string): number | null {
  ...
  const idx = KHADI_MONTH_SHARE.findIndex((row) => full.startsWith(row.m.toLowerCase()));
...
export function khadiHasBillUnits(data: ProposalData): boolean {
  return (data.bill.months ?? []).some((m) => m.units > 0);
}

export function khadiBillYearUnits(data: ProposalData): number {
  return (data.bill.months ?? []).reduce((sum, m) => sum + (m.units > 0 ? m.units : 0), 0);
}

export function khadiMonthlyForecast(data: ProposalData): KhadiMonthRow[] {
  const annual = khadiAnnualUnits(data);
  const annualSave = khadiAnnualSavings(data);
  ...
  return KHADI_MONTH_SHARE.map((row, i) => {
...
export type KhadiForecastNotes = {
  insightTag: string;
  insightTitle: string;
  insightBody: string;
...
export function khadiEffectiveSavingRate(data: ProposalData): number {
  const units = khadiAnnualUnits(data);
  const sav = khadiAnnualSavings(data);
 ...
 */
export function khadiForecastNotes(
  data: ProposalData,
  lang: "en" | "hi" = "en"
): KhadiForecastNotes {
  const hi = lang === "hi";
  ...
  const eng = khadiEngineeringModel(data);
  ...
  if (annual > 0) {
    parts.push(
      hi
        ? `Madder पट्टियाँ (${peakSpan}) monsoon से पहले की तेज़ धूप (GHI) हैं: सूरज ऊँचा रहता है, आसमान साफ़ रहता है, इसलिए array को साल की सबसे तेज़ रोशनी मिलती है।`
        : `Madder bars (${peakSpan}) are pre-monsoon high-GHI months: the sun sits high and skies are typically clear, so the array sees the year’s strongest irradiance.`
    );
    parts.push(
      hi
  ...
  const rate = khadiEffectiveSavingRate(data);
  return {
...
export type KhadiTermCard = { title: string; body: string };

export function khadiTermCards(data: ProposalData): KhadiTermCard[] {
  const raw = (data.terms.conditions ?? []).map((t) => t.trim()).filter(Boolean);
  const cards: KhadiTermCard[] = raw.slice(0, 4).map((text) => {
...
export function khadiHardwareRows(data: ProposalData): KhadiHardwareRow[] {
  ...
  const { count: panelCount, watt: panelWatt } = khadiParsePanelArray(data);
...
const KHADI_M2_PER_PANEL = 2.2;
const KHADI_MAX_VISUAL_PANELS = 24;
const KHADI_M2_TO_SQFT = 10.764;

export function khadiSqFtFromM2(m2: number): number {
  if (!(m2 > 0)) return 0;
  return Math.round(m2 * KHADI_M2_TO_SQFT);
}

/** e.g. ~18 m² (~194 sq ft). Empty when area is missing — never invents a roof size. */
export function formatKhadiAreaM2(m2: number): string {
  if (!(m2 > 0)) return "—";
  const sqft = khadiSqFtFromM2(m2);
  return `~${m2} m² (~${sqft.toLocaleString("en-IN")} sq ft)`;
}

function khadiParseMetricNumber(data: ProposalData, match: RegExp): number {
  const raw = khadiMetricValue(data, match);
  if (!raw) return 0;
  const n = Number(raw.replace(/[^\d.]/g, ""));
  return Number.isFinite(n) && n > 0 ? n : 0;
}

function khadiLiveCable(
  data: ProposalData,
  ...
  const dcM =
    Number(layout?.dcRunLengthM) > 0
      ...
      : khadiParseMetricNumber(data, /dc\s*run/i);
  const acM =
    Number(layout?.acRunLengthM) > 0
      ? Number(layout?.acRunLengthM)
      : khadiParseMetricNumber(data, /ac\s*run/i);
  const vdPct =
    Number(layout?.voltageDropDcPct) > 0
      ? Number(layout?.voltageDropDcPct)
      : khadiParseMetricNumber(data, /voltage\s*drop|\bvd\b/i);
  const bits: string[] = [];
...
function khadiEngineeringStandards(data: ProposalData): string[] {
...
function khadiMetricValue(data: ProposalData, match: RegExp): string {
  const row = (data.engineering.metrics ?? []).find((m) => match.test(m.label));
  return row?.value?.trim() ?? "";
}

function khadiParsePanelArray(data: ProposalData): { count: number; watt: number } {
...
export type KhadiEngineeringModel = {
  acKw: number;
  dcKwp: number;
  dcAcRatio: number;
  panelCount: number;
...
export type KhadiEngInsightCard = {
  title: string;
  body: string;
};

export function khadiEngineeringInsights(
  eng: KhadiEngineeringModel,
  lang: "en" | "hi" = "en"
): KhadiEngInsightCard[] {
  const hi = lang === "hi";
  const panelBit =
  ...
  const areaBit =
    eng.roofAreaM2 > 0
      ? `${formatKhadiAreaM2(eng.roofAreaM2)}`
      : hi
        ? "planning area"
        : "the planning area";
  const how =
  ...
  const roof: KhadiEngInsightCard = {
  ...
  const space: KhadiEngInsightCard = {
  ...
  const cable: KhadiEngInsightCard = {
...
export function khadiEngineeringModel(
  data: ProposalData,
  pptInput?: PremiumProposalPptInput | null
): KhadiEngineeringModel {
  const acKw = Number(data.meta.systemKw) || 0;
  const annual = khadiAnnualUnits(data);
  const { count, watt } = khadiParsePanelArray(data);
  ...
  const coverageRaw = khadiMetricValue(data, /load\s*coverage|coverage/i);
  ...
  const liveCity = data.engineering.cityLabel?.trim() || khadiLocation(data);
  const loc = liveCity;
  let siteLatLabel = "";
  ...
  const visualPanelCount = Math.min(Math.max(0, count), KHADI_MAX_VISUAL_PANELS);
  const roofAreaM2 = count > 0 ? Math.round(count * KHADI_M2_PER_PANEL) : 0;
  const cable = khadiLiveCable(data, pptInput);
  ...
  return {
    ...
    siteLatLabel,
    roofAreaM2,
    roofAreaLabel: formatKhadiAreaM2(roofAreaM2),
    m2PerPanel: KHADI_M2_PER_PANEL,
    m2PerPanelLabel: formatKhadiAreaM2(KHADI_M2_PER_PANEL),
    performanceRatioPct: acKw > 0 ? 75 : 0,
    peakSunHours: loc ? 5 : 0,
    specificYield,
    loadCoveragePct,
    standards: khadiEngineeringStandards(data),
