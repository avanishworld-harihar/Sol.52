...
export type KhadiContactDetails = {
  phone: string;
  email: string;
  website: string;
};
...
export function resolveKhadiContactDetails(
  data: ProposalData,
  pptWebsite?: string
): KhadiContactDetails {
...
export function useKhadiContactDetails(
  data: ProposalData,
  pptWebsite?: string
): KhadiContactDetails {
  const [contact, setContact] = useState(() =>
    resolveKhadiContactDetails(data, pptWebsite)
  );

  useEffect(() => {
    const sync = () => setContact(resolveKhadiContactDetails(data, pptWebsite));
    sync();
...
export function formatKhadiContactFooter(parts: string[]): string {
