/**
 * Lumina engineering arena — elevated GI MMS table layout.
 *
...
function pickKind(count: number): LuminaArenaKind {
...
export function luminaArenaLayout(count: number): LuminaArenaLayout {
  ...
  if (n <= 3) {
    const kind: LuminaArenaKind = n === 3 ? "3P" : n === 2 ? "2P" : "1P";
    return {
      rows: n,
      cols: 1,
