/**
 * Sienna engineering arena — elevated GI MMS table layout.
 *
...
export type SiennaArenaKind = "1P" | "2P" | "3P";

export type SiennaArenaLayout = {
  rows: number;
  cols: number;
  /** Row-major, north (top) → south (bottom). `false` is a reserved gap. */
  cells: boolean[];
  tableKind: SiennaArenaKind;
  heights: number[];
};
...
function pickKind(count: number): SiennaArenaKind {
...
export function siennaArenaLayout(count: number): SiennaArenaLayout {
  const n = Math.max(0, Math.floor(count));
  if (n <= 0) {
    return { rows: 0, cols: 0, cells: [], tableKind: "1P", heights: [] };
  }
  if (n <= 3) {
    const kind: SiennaArenaKind = n === 3 ? "3P" : n === 2 ? "2P" : "1P";
    return {
      rows: n,
      cols: 1,
