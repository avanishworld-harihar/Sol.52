/**
 * Sienna — live ProposalData only. No Harihar / Satna / 5 kW / ₹2.2L / Waaree fallbacks.
 */
...
export function siennaBrandParts(data: ProposalData): { head: string; tail: string } {
  const brand = siennaBrand(data);
...
export function siennaNetInvestment(data: ProposalData): number {
...
export function siennaAnnualUnits(data: ProposalData): number {
  return data.closing.annualUnits > 0 ? Math.round(data.closing.annualUnits) : 0;
}

export function siennaLocation(data: ProposalData): string {
  return data.meta.locationLine?.trim() || data.engineering.cityLabel?.trim() || "";
}

export function siennaIssueDate(generatedAt?: string): string {
...
export function formatSiennaKw(kw: number): string {
  if (!(kw > 0)) return "—";
  return kw % 1 === 0 ? String(kw) : kw.toFixed(1);
}

export function siennaMonthlySavings(data: ProposalData): number {
...
export function siennaAnnualSavings(data: ProposalData): number {
...
export function siennaYearlyBill(data: ProposalData): number {
...
export function siennaMonthlyBill(data: ProposalData): number {
  ...
  const yearly = siennaYearlyBill(data);
  return yearly > 0 ? Math.round(yearly / 12) : 0;
}

export function siennaBillMonths(data: ProposalData): ProposalData["bill"]["months"] {
  return (data.bill.months ?? []).filter((m) => m.units > 0 || m.netInr > 0);
}

export function siennaLifetime(data: ProposalData): number {
...
export type SiennaHwKind = "panel" | "inverter" | "structure" | "dcdb" | "acdb" | "la" | "earth";

export type SiennaHardwareRow = {
  kind: SiennaHwKind;
  role: string;
  title: string;
  detail: string;
...
const SIENNA_HW_CATALOG: Record<SiennaHwKind, SiennaHwCatalog> = {
  panel: {
    kind: "panel",
...
const SIENNA_HW_IMAGES: Array<{ match: RegExp; src: string }> = [
...
function siennaHardwareImage(hay: string): string {
  for (const row of SIENNA_HW_IMAGES) {
...
function filterPointsForKind(kind: SiennaHwKind, points: string[]): string[] {
...
function rowFromCatalog(
  ...
  const catalog = SIENNA_HW_CATALOG[kind];
  ...
  return {
    ...
    image: siennaHardwareImage(hay),
    accent: catalog.accent,
  };
}

/** Typical central-India rooftop share of year-1 yield. Applied only to live annual units. */
const SIENNA_MONTH_SHARE: ReadonlyArray<{ m: string; share: number; peak: boolean }> = [
...
function monthIndexFromLabel(label: string): number | null {
  ...
  const idx = SIENNA_MONTH_SHARE.findIndex((row) => full.startsWith(row.m.toLowerCase()));
...
export function siennaHasBillUnits(data: ProposalData): boolean {
  return (data.bill.months ?? []).some((m) => m.units > 0);
}

export function siennaBillYearUnits(data: ProposalData): number {
  return (data.bill.months ?? []).reduce((sum, m) => sum + (m.units > 0 ? m.units : 0), 0);
}

export function siennaMonthlyForecast(data: ProposalData): SiennaMonthRow[] {
  const annual = siennaAnnualUnits(data);
  const annualSave = siennaAnnualSavings(data);
  ...
  return SIENNA_MONTH_SHARE.map((row, i) => {
...
export type SiennaForecastNotes = {
  insightTag: string;
  insightTitle: string;
  insightBody: string;
...
export function siennaEffectiveSavingRate(data: ProposalData): number {
  const units = siennaAnnualUnits(data);
  const sav = siennaAnnualSavings(data);
 ...
 */
export function siennaForecastNotes(
  data: ProposalData,
  lang: "en" | "hi" = "en"
): SiennaForecastNotes {
  const hi = lang === "hi";
  ...
  const eng = siennaEngineeringModel(data);
  ...
  const rate = siennaEffectiveSavingRate(data);
  return {
...
export type SiennaTermCard = { title: string; body: string };

export function siennaTermCards(data: ProposalData): SiennaTermCard[] {
  const raw = (data.terms.conditions ?? []).map((t) => t.trim()).filter(Boolean);
  const cards: SiennaTermCard[] = raw.slice(0, 4).map((text) => {
...
export function siennaHardwareRows(data: ProposalData): SiennaHardwareRow[] {
  ...
  const { count: panelCount, watt: panelWatt } = siennaParsePanelArray(data);
...
const SIENNA_M2_PER_PANEL = 2.2;
const SIENNA_MAX_VISUAL_PANELS = 24;
const SIENNA_M2_TO_SQFT = 10.764;

export function siennaSqFtFromM2(m2: number): number {
  if (!(m2 > 0)) return 0;
  return Math.round(m2 * SIENNA_M2_TO_SQFT);
}

/** e.g. ~18 m² (~194 sq ft). Empty when area is missing — never invents a roof size. */
export function formatSiennaAreaM2(m2: number): string {
  if (!(m2 > 0)) return "—";
  const sqft = siennaSqFtFromM2(m2);
  return `~${m2} m² (~${sqft.toLocaleString("en-IN")} sq ft)`;
}

function siennaParseMetricNumber(data: ProposalData, match: RegExp): number {
  const raw = siennaMetricValue(data, match);
  if (!raw) return 0;
  const n = Number(raw.replace(/[^\d.]/g, ""));
  return Number.isFinite(n) && n > 0 ? n : 0;
}

function siennaLiveCable(
  data: ProposalData,
  ...
  const dcM =
    Number(layout?.dcRunLengthM) > 0
      ...
      : siennaParseMetricNumber(data, /dc\s*run/i);
  const acM =
    Number(layout?.acRunLengthM) > 0
      ? Number(layout?.acRunLengthM)
      : siennaParseMetricNumber(data, /ac\s*run/i);
  const vdPct =
    Number(layout?.voltageDropDcPct) > 0
      ? Number(layout?.voltageDropDcPct)
      : siennaParseMetricNumber(data, /voltage\s*drop|\bvd\b/i);
  const bits: string[] = [];
...
function siennaEngineeringStandards(data: ProposalData): string[] {
...
function siennaMetricValue(data: ProposalData, match: RegExp): string {
  const row = (data.engineering.metrics ?? []).find((m) => match.test(m.label));
  return row?.value?.trim() ?? "";
}

function siennaParsePanelArray(data: ProposalData): { count: number; watt: number } {
...
export type SiennaEngineeringModel = {
  acKw: number;
  dcKwp: number;
  dcAcRatio: number;
  panelCount: number;
...
export type SiennaEngInsightCard = {
  title: string;
  body: string;
};

export function siennaEngineeringInsights(
  eng: SiennaEngineeringModel,
  lang: "en" | "hi" = "en"
): SiennaEngInsightCard[] {
  const hi = lang === "hi";
  const panelBit =
    eng.panelCount > 0
  ...
  const areaBit =
    eng.roofAreaM2 > 0
      ? `${formatSiennaAreaM2(eng.roofAreaM2)}`
      : hi
        ? "planning area"
        : "the planning area";
  const how =
  ...
  const roof: SiennaEngInsightCard = {
  ...
  const space: SiennaEngInsightCard = {
  ...
  const cable: SiennaEngInsightCard = {
...
export function siennaEngineeringModel(
  data: ProposalData,
  pptInput?: PremiumProposalPptInput | null
): SiennaEngineeringModel {
  const acKw = Number(data.meta.systemKw) || 0;
  const annual = siennaAnnualUnits(data);
  const { count, watt } = siennaParsePanelArray(data);
  ...
  const coverageRaw = siennaMetricValue(data, /load\s*coverage|coverage/i);
  ...
  const liveCity = data.engineering.cityLabel?.trim() || siennaLocation(data);
  const loc = liveCity;
  let siteLatLabel = "";
  ...
  const visualPanelCount = Math.min(Math.max(0, count), SIENNA_MAX_VISUAL_PANELS);
  const roofAreaM2 = count > 0 ? Math.round(count * SIENNA_M2_PER_PANEL) : 0;
  const cable = siennaLiveCable(data, pptInput);
  ...
  return {
    ...
    siteLatLabel,
    roofAreaM2,
    roofAreaLabel: formatSiennaAreaM2(roofAreaM2),
    m2PerPanel: SIENNA_M2_PER_PANEL,
    m2PerPanelLabel: formatSiennaAreaM2(SIENNA_M2_PER_PANEL),
    performanceRatioPct: acKw > 0 ? 75 : 0,
    peakSunHours: loc ? 5 : 0,
    specificYield,
    loadCoveragePct,
    standards: siennaEngineeringStandards(data),
