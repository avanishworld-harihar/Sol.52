...
export type SiennaContactDetails = {
  phone: string;
  email: string;
  website: string;
};
...
export function resolveSiennaContactDetails(
  data: ProposalData,
  pptWebsite?: string
): SiennaContactDetails {
...
export function useSiennaContactDetails(
  data: ProposalData,
  pptWebsite?: string
): SiennaContactDetails {
  const [contact, setContact] = useState(() =>
    resolveSiennaContactDetails(data, pptWebsite)
  );

  useEffect(() => {
    const sync = () => setContact(resolveSiennaContactDetails(data, pptWebsite));
    sync();
...
export function formatSiennaContactFooter(parts: string[]): string {
