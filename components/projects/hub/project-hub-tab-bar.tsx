...
export function ProjectHubTabBar({
  ...
  return (
    ...
    >
      <div className="grid w-full min-w-0 grid-cols-3 gap-px rounded-md border border-slate-200/80 bg-white/90 p-px shadow-sm backdrop-blur-md sm:grid-cols-6 sm:gap-0.5 sm:rounded-xl sm:p-1 dark:border-white/10 dark:bg-[#0c1017]/90">
        {TAB_CONFIG.map((tab) => {
          ...
          return (
            <button
              ...
              className={cn(
                "relative flex min-w-0 items-center justify-center gap-0.5 rounded px-1 py-0.5 text-[9px] font-semibold transition-colors duration-150 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-slate-400 focus-visible:ring-offset-1 dark:focus-visible:ring-slate-500 max-sm:min-h-[1.75rem] sm:gap-1.5 sm:rounded-lg sm:px-3 sm:py-1.5 sm:text-[12px]",
                isActive
