...
export function ProposalTemplateThumbnail({ variant, className, size = "card" }: Props) {
  ...
  return (
    <div
      ...
      aria-hidden
    >
      {variant === "onyx" ? <OnyxThumb size={size} /> : null}
...
function OnyxThumb({ size }: { size: "card" | "preview" }) {
  const splitW = size === "preview" ? "w-[28%]" : "w-[30%]";
  return (
    <div className="flex h-full w-full items-center justify-center bg-[#f8f8f8]">
      <div className={cn("relative flex h-[58%] w-[72%] overflow-hidden rounded-[3px] border border-black/5 shadow-sm", size === "preview" && "h-[62%] w-[68%]")}>
        <div className={cn("h-full bg-[#121212]", splitW)} />
        <div className="flex flex-1 flex-col justify-center bg-[#f8f8f8] p-[8%]">
          <div className={cn("font-serif italic text-[#1a1a1a]", size === "preview" ? "text-[10px]" : "text-[4px]")}>
            Name
          </div>
          <div className={cn("mt-[8%] bg-[#B87333]", size === "preview" ? "h-[2px] w-[20%]" : "h-[1px] w-[22%]")} />
          <div className={cn("mt-[10%] text-[#666]", size === "preview" ? "text-[7px]" : "text-[3px]")}>Body text</div>
        </div>
      </div>
    </div>
  );
