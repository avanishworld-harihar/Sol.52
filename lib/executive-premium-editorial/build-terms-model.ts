...
function termsConditions(lang: ProposalLang): string[] {
  if (lang === "hi") {
    return [
...
function documentsRequired(lang: ProposalLang): string[] {
  if (lang === "hi") {
    return [
...
export function buildEditorialTermsModel(
  ...
  const amcCostParagraph =
    lang === "hi"
      ? `पहले ${freeAmcYears} year${freeAmcYears > 1 ? "s" : ""} का AMC quoted price में शामिल। Year ${freeAmcYears + 1} से annual maintenance invoice value (₹${invoiceRef}) का 2% पर charge हो सकता है, 5% year-on-year escalation के साथ, signed O&M agreement के अधीन।`
      : `First ${freeAmcYears} year${freeAmcYears > 1 ? "s" : ""} AMC is included in the quoted price. From Year ${freeAmcYears + 1} onwards, annual maintenance may be charged at 2% of invoice value (₹${invoiceRef}) with 5% year-on-year escalation, subject to a signed O&M agreement.`;
  ...
  return {
    ...
    terms_conditions: termsConditions(lang),
    documents_required: documentsRequired(lang),
    amc_objective: amcObjective,
    amc_scope: amcScope,
    client_scope: clientScope,
    amc_cost_paragraph: amcCostParagraph,
    amc_terms: amcTerms,
  };
}
