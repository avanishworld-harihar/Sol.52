...
• LAST SIX MONTHS CONSUMPTION table (older 1-page layout, lower section): columns **Bill Month | Date | Reading | Unit**. Copy the **Unit** column only.
• CONSUMPTION HISTORY table (newer 2-page MPPKVVCL layout, page 1, under the "CONSUMPTION TREND" bar chart): SAME data, different columns — **Bill Month | Bill Date | Reading | Units | MD | PF** (6 columns). Copy ONLY the **Units** column (4th column, e.g. "JUN-2026 | 01/07/2026 | 3443.16 | 492.10 | 3.25 | 0.98" → 492.10). NEVER the Reading accumulator (3443.16, 3rd column) and NEVER the small MD (3.25, 5th column — Max Demand in kW, not units) or PF (0.98, 6th column — Power Factor 0–1). The per-row "Bill Date" column (billing date of THAT historical month) is NOT the same as the bill's own top-of-page "BILL DATE" (issue date of the current bill) — do not confuse the two.
• EITHER history table (old OR new layout) contains 5 recent consecutive previous months PLUS one 6th row that is the "same month last year" entry (often the oldest-looking row, one full year before the CURRENT bill month, breaking the otherwise-consecutive sequence). Example: bill_month=JUL-2026 → table shows JUN-2026, MAY-2026, APR-2026, MAR-2026, FEB-2026 (5 consecutive), then JUL-2025 (same month, prior year — NOT JAN-2026, which would be the true 6th consecutive month). This JUL-2025 row is real historical data, not a duplicate/error — include it in consumption_history normally.
• Newer 2-page layout, PAGE 2 sections (do not treat these as new/extra months): "CURRENT READING DETAILS" restates ONLY the current bill month's meter reading/MD/PF — never add it as a separate history row. "DETAILED BILL BREAKUP" / "Billing Charges Information" repeats the same charge line items as page 1 — extract charges from wherever they are legible, do not double-count. "TOD DETAILS" (TOD1–TOD4 rows + pie chart) maps to tod_rebate_inr exactly like the older bill's simpler "TOD Consumption Detail" table (see rule 12 below) — still LT/domestic, do NOT populate HT-only tod_units/tod_amounts_inr from it. "PAYMENT HISTORY" (last payment month/amount/ref) is NOT the current bill amount — never use it for current_month_bill_amount_inr or total_amount_payable_inr.
