...
export function compileProposalDocument(
  ...
  const presetId =
    options?.presetId && isValidPresetId(options.presetId)
      ? options.presetId
      : normalizePresetId(options?.presetId ?? null);

  const layout = resolveProposalLayout(input, presetId);
