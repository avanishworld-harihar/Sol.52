...
• LAST SIX MONTHS CONSUMPTION table (older 1-page layout, lower section): Columns are typically **Bill Month | Date | Reading | Unit**. Copy the **Unit** column only (e.g. MAY-2026 → 990), NEVER the Reading accumulator (e.g. 20957).
• CONSUMPTION HISTORY table (newer 2-page MPPKVVCL layout, page 1, under the "CONSUMPTION TREND" bar chart): SAME kind of data, different columns — **Bill Month | Bill Date | Reading | Units | MD | PF** (6 columns). Copy ONLY the **Units** column (4th column, e.g. "JUN-2026 | 01/07/2026 | 3443.16 | 492.10 | 3.25 | 0.98" → 492.10). NEVER the Reading accumulator (3443.16, 3rd column), and NEVER the small MD (3.25, 5th column — Max Demand in kW) or PF (0.98, 6th column — Power Factor 0–1), which sit right next to Units and can be mistaken for it. The per-row "Bill Date" column (billing date of THAT historical month) is a different field from the bill's own top-of-page "BILL DATE" (issue date of the current bill) — do not confuse the two.
• EITHER history table (old or new layout) contains 5 recent previous months PLUS one "same month last year" entry (often shown in bold, or the row that breaks the otherwise-consecutive month sequence) as a year-on-year comparison reference. Example: if bill_month=JUN-2026, this table shows JAN-2026, FEB-2026, MAR-2026, APR-2026, MAY-2026 (5 consecutive) plus JUN-2025 (same month last year — NOT DEC-2025, which would be the true 6th consecutive month). The JUN-2025 row is historical context only — put it in consumption_history but NEVER into months.jun. This row is real data, not a duplicate/error.
• Newer 2-page layout, PAGE 2 sections — do not treat these as extra months or duplicate charges: "CURRENT READING DETAILS" restates ONLY the current bill month's meter reading/MD/PF, never a new history row. "DETAILED BILL BREAKUP" / "Billing Charges Information" repeats the SAME charge line items already on page 1 — extract each charge once, from whichever page is legible. "TOD DETAILS" (TOD1–TOD4 rows + pie chart) maps to tod_rebate_inr exactly like the older bill's simpler "TOD Consumption Detail" table (see rule 11 below) — still LT/domestic, do NOT populate HT-only tod_units/tod_amounts_inr from it. "PAYMENT HISTORY" (last payment month/amount/ref) is NOT the current bill amount — never source current_month_bill_amount_inr or total_amount_payable_inr from it.
• "MESSAGES / NOTES" free-text banner (e.g. security-deposit installment adjustments spread over future months) is informational only — NEVER extract numbers from it into any charge field or consumption_history.
